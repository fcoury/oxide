...
use crate::wire::{UnknownMessageKindError, CHECKSUM_PRESENT, HEADER_SIZE, OP_MSG};
...
impl Replyable for OpMsg {
    fn reply(&self, request_id: u32, doc: Document) -> Result<Vec<u8>, UnknownMessageKindError> {
        ...
        if self.sections.len() > 0 && self.sections[0].kind == 0 {
            return Ok(OpMsg::new_with_body_kind(header, self.flags, self.checksum, doc).to_vec());
